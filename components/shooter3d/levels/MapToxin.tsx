...
import {
  ...
  WALL_H, EMISSIVE_SICK, EMISSIVE_RUST, EMISSIVE_FIRE, TRIM_COLOR,
} from "./LevelBase";
...
function MapToxinImpl() {
  ...
  return (
    <group>
      ...
      <Wall position={[4, WALL_H/2, 0]} size={[0.5, WALL_H, D - 4]} texture={wallTex} />
      <GlowStrip position={[-3.7, 0.15, 0]} scale={[0.08, 0.08, D - 6]} color={EMISSIVE_SICK} />
      <GlowStrip position={[3.7, 0.15, 0]} scale={[0.08, 0.08, D - 6]} color={EMISSIVE_SICK} />

      {/* ═══ E-W CORRIDOR ═══ */}
      <Wall position={[0, WALL_H/2, -4]} rotation={[0, Math.PI/2, 0]} size={[0.5, WALL_H, W - 4]} texture={wallTex} />
      <Wall position={[0, WALL_H/2, 4]} rotation={[0, Math.PI/2, 0]} size={[0.5, WALL_H, W - 4]} texture={wallTex} />
      <GlowStrip position={[0, 0.15, -3.7]} scale={[W - 6, 0.08, 0.08]} color={EMISSIVE_SICK} />
      <GlowStrip position={[0, 0.15, 3.7]} scale={[W - 6, 0.08, 0.08]} color={EMISSIVE_SICK} />
      ...
      <Crate position={[-28, 0.7, -34]} size={[2.5, 1.4, 2.5]} stripeColor={EMISSIVE_SICK} />
      <SpawnPortal position={[-30, 0, -35]} color={EMISSIVE_SICK} />
      ...
      <Wall position={[38, WALL_H/2, -35]} size={[0.5, WALL_H, 10]} texture={wallTex} />
      <Wall position={[22, WALL_H/2, -35]} size={[0.5, WALL_H, 10]} texture={wallTex} />
      <Crate position={[28, 0.7, -36]} size={[2.5, 1.4, 2.5]} stripeColor={EMISSIVE_RUST} />
      <SpawnPortal position={[30, 0, -35]} color={EMISSIVE_RUST} />
      ...
      <Wall position={[-38, WALL_H/2, 35]} size={[0.5, WALL_H, 10]} texture={wallTex} />
      <Wall position={[-22, WALL_H/2, 35]} size={[0.5, WALL_H, 10]} texture={wallTex} />
      <SpawnPortal position={[-30, 0, 35]} rotation={[0, Math.PI, 0]} color={EMISSIVE_SICK} />
      ...
      <Wall position={[38, WALL_H/2, 35]} size={[0.5, WALL_H, 10]} texture={wallTex} />
      <Wall position={[22, WALL_H/2, 35]} size={[0.5, WALL_H, 10]} texture={wallTex} />
      <SpawnPortal position={[30, 0, 35]} rotation={[0, Math.PI, 0]} color={EMISSIVE_RUST} />
      ...
      {/* ═══ TOXIC VATS (decorative cylinders) ═══ */}
      {[[-30,-10],[30,-10],[-30,10],[30,10]].map(([x,z],i) => (
        <RigidBody key={`vat-${i}`} type="fixed" colliders="cuboid">
          <mesh position={[x, 1, z]}>
            <cylinderGeometry args={[1.5, 1.5, 2, 10]} />
            <meshStandardMaterial color={EMISSIVE_SICK} emissive={EMISSIVE_SICK} emissiveIntensity={0.5} toneMapped={false} transparent opacity={0.7} />
          </mesh>
        </RigidBody>
      ))}

      {/* ═══ ELEVATED PIPE CATWALKS ═══ */}
      <Catwalk position={[0, 2, -25]} size={[4, 0.5, 12]} glowColor={EMISSIVE_SICK} />
      <Ramp position={[0, 1, -18]} rampRotation={[-0.2, 0, 0]} size={[3, 0.5, 4]} />
      <Catwalk position={[0, 2, 25]} size={[4, 0.5, 12]} glowColor={EMISSIVE_SICK} />
      <Ramp position={[0, 1, 32]} rampRotation={[0.2, 0, 0]} size={[3, 0.5, 4]} />

      {/* ═══ CORRIDOR COVER ═══ */}
      <Crate position={[0, 0.7, -18]} size={[2, 1.4, 2]} stripeColor={EMISSIVE_SICK} />
      <Crate position={[0, 0.7, 18]} size={[2, 1.4, 2]} stripeColor={EMISSIVE_SICK} />
      <Crate position={[-20, 0.7, 0]} size={[2, 1.4, 2]} stripeColor={EMISSIVE_RUST} />
      <Crate position={[20, 0.7, 0]} size={[2, 1.4, 2]} stripeColor={EMISSIVE_RUST} />

      {/* Extra spawn portals at corridor ends */}
      <SpawnPortal position={[0, 0, -HD + 3]} color={EMISSIVE_SICK} />
      <SpawnPortal position={[0, 0, HD - 3]} rotation={[0, Math.PI, 0]} color={EMISSIVE_SICK} />

      {/* ═══ LIGHTING ═══ */}
      <CeilingLight position={[0, WALL_H - 0.1, 0]} color={EMISSIVE_SICK} intensity={3} />
      <CeilingLight position={[0, WALL_H - 0.1, -30]} color={EMISSIVE_SICK} flicker />
      <CeilingLight position={[0, WALL_H - 0.1, 30]} color={EMISSIVE_SICK} />
      <CeilingLight position={[-30, WALL_H - 0.1, 0]} color={EMISSIVE_RUST} flicker />
