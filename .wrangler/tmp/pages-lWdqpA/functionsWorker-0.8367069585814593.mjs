...
async function queryAnalytics(token, accountTag, siteTag) {
  ...
  const totalsData = totals.data?.viewer.accounts[0]?.rumPageloadEventsAdaptiveGroups[0];
  const dailyData = daily.data?.viewer.accounts[0]?.rumPageloadEventsAdaptiveGroups ?? [];
  const referrersData = referrers.data?.viewer.accounts[0]?.rumPageloadEventsAdaptiveGroups ?? [];
  const pagesData = pages.data?.viewer.accounts[0]?.rumPageloadEventsAdaptiveGroups ?? [];
  return {
    ...
    topReferrers: referrersData.filter((r) => r.dimensions.refererHost).map((r) => ({
      host: r.dimensions.refererHost,
      views: r.count
    })),
    topPages: pagesData.filter((p) => p.dimensions.requestPath).map((p) => ({
      path: p.dimensions.requestPath,
      views: p.count
    }))
  };
}
__name(queryAnalytics, "queryAnalytics");
...
var onRequest2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    if (method === "GET") {
      const data = await kv.get(SCORES_KEY, "json");
...
// ../.wrangler/tmp/pages-lWdqpA/functionsRoutes-0.8751961263776253.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-csK7AI/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-csK7AI/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8367069585814593.mjs.map
