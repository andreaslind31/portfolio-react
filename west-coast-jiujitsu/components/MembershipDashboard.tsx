...
export default function MembershipDashboard() {
  ...
  const fetchMemberships = useCallback(async () => {
    try {
      ...
      if (!res.ok) throw new Error("Kunde inte hämta medlemskap");
      const data = await res.json();
      setMemberships(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Kunde inte ladda medlemskap");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handleCreate(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || "Kunde inte skapa medlemskap");
      }
  ...
  async function handleUpdate(
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || "Kunde inte uppdatera medlemskap");
      }
      await fetchMemberships();
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || "Kunde inte ta bort medlemskap");
      }
      await fetchMemberships();
    } catch (err) {
    ...
    <div className="membership-dashboard">
      ...
      <div className="membership-stats">
        <div className="membership-stats__card membership-stats__card--total">
          ...
          <span className="membership-stats__label">Totalt medlemmar</span>
        </div>
        <div className="membership-stats__card membership-stats__card--active">
          ...
          <span className="membership-stats__label">Aktiva</span>
        </div>
        <div className="membership-stats__card membership-stats__card--warning">
          ...
          <span className="membership-stats__label">Går ut snart</span>
        </div>
        <div className="membership-stats__card membership-stats__card--danger">
          ...
          <span className="membership-stats__label">Utgångna</span>
        </div>
      </div>
      ...
      <div className="membership-toolbar">
        <div className="membership-toolbar__left">
          ...
          >
            ...
            <option value="expiring-soon">Går ut snart</option>
            <option value="expiring">Går ut denna vecka</option>
          ...
          >
            <option value="expirationDate-asc">Utgångsdatum (närmast)</option>
            <option value="expirationDate-desc">Utgångsdatum (längst bort)</option>
            <option value="name-asc">Namn (A-Ö)</option>
            <option value="name-desc">Namn (Ö-A)</option>
            <option value="membershipSince-asc">Medlem sedan (äldst)</option>
            <option value="membershipSince-desc">Medlem sedan (nyast)</option>
            <option value="beltRank-asc">Bälte (vitt till svart)</option>
            <option value="beltRank-desc">Bälte (svart till vitt)</option>
          </select>
        </div>
        <button
      ...
      {filtered.length === 0 ? (
        <div className="membership-empty">
          {memberships.length === 0 ? (
            <>
              <p className="membership-empty__title">Inga medlemmar ännu</p>
              <p>Klicka på &quot;+ Lägg till medlem&quot; för att börja hantera dina medlemskap.</p>
            </>
          ) : (
            <p>Inga medlemmar matchar din sökning eller filtrering.</p>
          )}
        </div>
      ) : (
